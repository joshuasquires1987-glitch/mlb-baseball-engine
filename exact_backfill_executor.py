...
class ExactBackfillExecutor:
    def __init__(self,boxscore_fetcher,summary_fetcher):
        self.boxscore_fetcher=boxscore_fetcher
    ...
    def ingest_game(self,game_pk,game_date):
        ...
        if key in self.processed:
            return
        box=self.boxscore_fetcher(game_pk)
        ...
        summary=self.summary_fetcher(game_pk)
    ...
    def ingest_many(self,game_refs):
        for g in sorted(game_refs,key=lambda x:x["game_date"]):
            self.ingest_game(g["game_pk"],g["game_date"])
    ...
    def export_bundle(self):
        return {
            "pitching_rows":self.pitching_store.pitching_rows,
            "team_games":self.team_games,
            "processed_game_pks":sorted(self.processed),
        }
