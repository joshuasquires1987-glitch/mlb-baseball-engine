        ...
        for g in sorted(games,key=lambda x:(x.get("game_time_utc") or "",x["game_pk"])):
            ...
            if ar["usable"]:
                try:
                    ...
                    rows.append(build_training_row(game,ps,park["park_factor"],snap))
                except Exception as e:
                    skipped.append({"game_pk":g["game_pk"],"reason":f"build-error:{type(e).__name__}"})
            else:
                skipped.append({"game_pk":g["game_pk"],"reason":"|".join(ar["missing"])})

            if venue is not None and venue.get("utc_offset_hours") is not None and g.get("game_time_utc") is not None:
                stamp={
