...
class StarterStateCalculator:
    ...
    def calculate(self,starts,as_of_date):
        prior=[r for r in starts if r["date"] < as_of_date]
        if not prior:
            return {
                "talent_score":0.0,
                "expected_outs":self.league_outs_per_start,
                "starts_prior":0,
        ...
        # Lower run rate is better.
        talent_score=bounded_score(
            ra_per_bf,
            self.league_runs_per_bf,
            max(self.league_runs_per_bf*0.20,1e-6),
            invert=True
        )
